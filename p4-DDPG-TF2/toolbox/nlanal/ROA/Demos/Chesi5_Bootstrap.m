%------------------------------------------------------------------
% Chesi example 5 with Bootstrap
%   Compute an estimate of the region of attraction for Chesi 
%   example 5 using the bootstrap method.  The V-S iteration is run with
%   a quadratic Lyapunov function and the shape function p(x)=x'*x.  The
%   iteration is initialized with the Lyapunov function from linearized 
%   analysis.  Then the iteration is run again with the shape function set
%   equal to the quadratic Lyapunov function obtained during the first
%   iteration.
%
% References: 
...
x1dot = x2;
x2dot = x3;
x3dot = -4*x1-3*x2-3*x3+x1^2*x2+x1^2*x3;
f = [x1dot; x2dot;x3dot];
