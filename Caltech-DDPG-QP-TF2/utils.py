...
def HESSIAN_VECTOR_PRODUCT(self, y, action_comp):
    with tf.GradientTape(persistent=True) as tape2:
        with tf.GradientTape(persistent=True) as tape1:
            value_pred = self.mlp_model(self.observation, training=True)
            loss = tf.math.reduce_mean(tf.math.pow(action_comp - value_pred, 2)) 
        first_derivative = tape1.gradient(loss, self.mlp_model.trainable_variables)
        flat_y = list()
        start = 0
        for var in vrbs:
            ...
            flat_y.append(param)
            start += variable_size
        gradient_with_y = [tf.math.reduce_sum(f_d * f_y) for (f_d, f_y) in zip(first_derivative, flat_y)]
    grads = tape2.gradient(gradient_with_y, self.mlp_model.trainable_variables)
    HVP = tf.concat(values=[tf.reshape(g, [np.prod(v.get_shape().as_list()),]) for (g,v) in zip(grads, self.mlp_model.trainable_variables)], axis=0)
    ...
    grads = tape.gradient(loss, self.mlp_model.trainable_variables)
    gradient_objective = tf.concat(values=[tf.reshape(g, [np.prod(v.get_shape().as_list()),]) for (g,v) in zip(grads, self.mlp_model.trainable_variables)], axis=0)
