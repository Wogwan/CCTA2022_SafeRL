...
% nonlinear baseline model. This is done in the following way: 
%
...
% ---------- Betadot 
% 
H1 = [a0;a22*p ;a22*xcB ;a22*q ;a22*r];
pbetadot = polydecvar('a',H1,'vec'); 
xbetadot = [beta;       alpha;      p;     q;      r;      phi;        xcB];
Xdatabeta = [betadata; alphadata; pdata;  qdata;  rdata;  phidata;  xcBdata];
Ydatabeta = xcldot2(1,:); 
...
% ---------- pdot 
% 
H3 = [a2; p*q ; q*r; a11*p; a11*r; a11*xcB];
ppdot = polydecvar('c', H3,'vec'); 
xpdot = [beta;alpha;p;q;r;xcB]; 
Xdatap = [betadata;alphadata;pdata;qdata;rdata;xcBdata];
Ydatap = xcldot2(3,:);
...
% ---------- rdot 
% 
H5 = [a2 ; p*q ; q*r; a11*p; a11*r; a11*xcB]; 
prdot = polydecvar('c', H5,'vec'); 
xrdot = [beta;alpha;p;q;r;xcB]; 
Xdatar = [betadata;alphadata;pdata;qdata;rdata;xcBdata];
Ydatar = xcldot2(5,:);
...
xcldotB = [pbetafit; palphafit; ppfit; pqfit ;  prfit; pphifit; 0] + CLB.A*x;

% --------------- Save Data
V2 = strcat('2_Mar14_PolyF18BaselineModel_Phi_',num2str(phi0*r2d));
save(V2,'xcldotB','CLB','xtrim','utrim','Pss','Xdata','x')
