function y = sos_compute_nosim(u,input_1,input_2)
...
% f2 = x1^5/12 -5/3*x1^3+10*x1+a_rl;
f2 = 0.070421222500154631585012054983963*x1^5+0.00000000000000071054273576010022531416952862872*x1^4-1.6415384826007917012935521938743*x1^3-0.0000000000000021316282072803006759425085858861*x1^2+9.9876513003548918895324604250163*x1+0.0000000000000015987211554602254576530748631014;
...
while 1
    ...
    %% Find optimal L1
    L1 = sos_feasu_nosim(sys,f2);
    %% Find minimize U
    u_out = sos_min_u_sosp_nosim(sys, L1, f2);
    if sys.input_limit <= 2 || isequal(u_out,0)
        break
    else
        u_store = u_out;
        continue
...
syms x1 x2
u_sos = p2s(u_store);
y = double(subs(u_sos, {x1,x2}, {input_1,input_2}));
end
