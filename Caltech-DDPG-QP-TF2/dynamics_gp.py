...
def get_dynamics(self, obs, u_rl):
    ...
    # g(st) in h(s), and multiply by action
    g = np.array([3/(m*l**2)*dt**2, 3/(m*l**2)*dt])
    x = np.array([theta, theta_dot]) # states at the current step. 
...
def update_GP_dynamics(self,path):
    ...
    X = path['Observation']
    X = X.reshape((self.max_episode_len, self.observation_size)) # (200, 3)
    ...
    for i in range(L-1):
        ...
        # f generates theta and theta_dot of the next step from the current status and actions.
        # Get the difference between actual environment and predicted environment.
        err[i,:] = np.array([theta, theta_dot]) - f 
    # Update GP model for theta and theta_dot.
...
def get_GP_dynamics(self, obs, u_rl):
    ...
    f = np.zeros(2)
    # Based on the current states, GP models predict the uncertainty factor in dynamics.
